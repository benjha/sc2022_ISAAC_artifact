...
#    endif
            ...
            void writeTimes(uint64_t time)
            {
                if(rank == 0)
                {
                    ...
                    uint64_t max = 0;
                    uint64_t average = 0;
                    uint64_t times[numProc];
                    MPI_Gather(&time, 1, MPI_UINT64_T, times, 1, MPI_UINT64_T, 0, MPI_COMM_WORLD);
                ...
                {
                    MPI_Gather(&time, 1, MPI_UINT64_T, NULL, 0, MPI_UINT64_T, 0, MPI_COMM_WORLD);
                }
            }

	    void writeTimes(double time)
            {
                if(rank == 0)
                {
		    ...
		    uint64_t microSeconds = 1000000lu;
                    ...
                    average /= numProc;
                    timingsFile << (min*microSeconds) << "," << (max*microSeconds) << "," << (average*microSeconds) << ",";
                }
                else
                {
                    MPI_Gather(&time, 1, MPI_DOUBLE, NULL, 0, MPI_DOUBLE, 0, MPI_COMM_WORLD);
                }
            }
            ...
            void benchmark(bool pause)
            {
                if(recording && !pause && runSteps >= 0)
                {
                    ...
                    writeTimes(visualization->icetCompositeTime);
		    writeTimes(visualization->icetBlendTime);
		    writeTimes(visualization->icetTotalDrawTime);
        ...
        private:
            ...
            void pluginLoad() override
            {
                if(!notifyPeriod.empty())
                {
					...
					<< "min-icetCompositeTime,max-icetCompositeTime,avg-icetCompositeTime,"
					<< "min-icetBlendTime,max-icetBlendTime,avg-icetBlendTime,"
					<< "min-icetTotalDrawTime,max-icetTotalDrawTime,avg-icetTotalDrawTime,"
